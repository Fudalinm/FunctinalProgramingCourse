...
rabinCreateCryptedFile inpStr e n = [toEnum (rabinGetLastSize inpStr)] ++ (rabinNumbersToFile (rabinCrypt e n(rabinSplitFileIntoNumbers inpStr)) (ceiling(logBase (fromIntegral 256) (fromIntegral n) ) ) )
...
f1 0 acc _  = acc
f1 k acc [] = acc
f1 n acc (x:xs)= f1 (n-1) (acc+(toInteger(fromEnum x))) xs
...
rabinNumbersToFile :: [Integer] -> Int -> String
...
encrypt = do
	...
	putStr("to bedzie e:" ++ (show e) ++ "\n")
	putStr("to bedzie d:" ++ (show d) ++ "\n") -- jes prywatny bo sluzy do de szyfrowania
